...
!text --content="A video is a combo of SCENES" --size=100 --color=white --animation=fadeInSlideDown
...
!text --content="This is another SCENE with Image" --size=80 --color=white --animation=fadeInOnly
...
## !scene --duration=4 --title=layout --background=indigo
!transition --type=slide --duration=0.3 --direction=from-right
...
## !scene --duration=3 --title=code-intro --background=black
!transition --type=slide --duration=0.75 --direction=from-bottom
\`\`\`js ! --theme=midnight  --align=top --fontSize=32
const markdownToVideo = () => {
  const fn = () => {
  // !highlight --color=green --duration=1
   // we can have codeblock as well xD
  }
  return fn
}
...
## !scene --duration=4 --title=coming-soon --background=https://images.unsplash.com/photo-1525453719223-4e781eb83a4c?q=80&w=2071&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D
!text --content="Coming Soon 🕊️" --size=120 --color=white --animation=fadeInOnly
