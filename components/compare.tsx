...
export const Compare = ({
  ...
  const startAutoplay = useCallback(() => {
    ...
    const animate = () => {
      ...
      const percentage = progress <= 1 ? progress * 80 : (2 - progress) * 80;
  ...
  const handleMove = useCallback(
    (clientX: number) => {
      ...
      if (slideMode === "hover" || (slideMode === "drag" && isDragging)) {
        ...
        const percent = (x / rect.width) * 80;
        requestAnimationFrame(() => {
          setSliderXPercent(Math.max(0, Math.min(80, percent)));
        });
      }
    },
  ...
  return (
    ...
    >
      <AnimatePresence initial={false}>
        <motion.div
          className="absolute top-0 z-30 m-auto h-full w-px border bg-gradient-to-b from-transparent from-[5%] via-indigo-500 to-transparent to-[95%]"
          style={{
            left: `${sliderXPercent}%`,
            top: "0",
            zIndex: 40,
      ...
      <AnimatePresence initial={false}>
        <motion.div className="absolute left-0 top-0 z-[19] h-full w-full select-none">
          {secondElement}
        </motion.div>
      </AnimatePresence>
    </div>
